...
public interface SaleItemRepository extends MainRepository<SaleItems, Long> {
    ...
    @Query("SELECT new octagon.retail.model.sale.TopTenItems(sum(s.qty) , s.itemBarCode ,s.itemName) FROM SaleItems s where s.isDeleted = false group by s.itemBarCode , s.itemName  order by sum(s.qty) desc limit 10")
